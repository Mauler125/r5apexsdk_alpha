...
void CBanSystem::AddEntry(std::string ipAddress, std::int64_t originID)
{
...
void CBanSystem::DeleteEntry(std::string ipAddress, std::int64_t originID)
{
...
void CBanSystem::AddConnectionRefuse(std::string error, int userID)
{
	if (refuseList.empty())
	{
...
void CBanSystem::DeleteConnectionRefuse(int userID)
{
	for (int i = 0; i < refuseList.size(); i++) // Loop through vector.
	{
		if (refuseList[i].second == userID) // Do any entries match our vector?
...
bool CBanSystem::IsBanned(std::string ipAddress, std::int64_t originID)
{
	for (int i = 0; i < banList.size(); i++)
	{
